...
(
    ...
    `file_path`   varchar(512) NOT NULL COMMENT 'bug所在文件路径',
    `class_name`  varchar(256)  DEFAULT NULL COMMENT 'bug所在类名',
    `method_name` mediumtext COMMENT 'bug所在方法名',
    `rawIssue_id` varchar(36)  NOT NULL COMMENT 'bug所属rawissueID',
    `code`        longtext COMMENT 'bug源代码',
    PRIMARY KEY (`uuid`) USING BTREE,
    UNIQUE KEY `uuid` (`uuid`) USING BTREE,
    KEY           `raw_issue_id_index` (`rawIssue_id`) USING BTREE
) ENGINE = InnoDB
