...
import cn.edu.fudan.violation.domain.enums.IssueStatusEnum;
 ...
 */
@Slf4j
public class NormalMatcher extends BaseMatcher implements Matcher {
    ...
    public void sumUpRawIssues(String parentCommit, List<RawIssue> preRawIssues, List<RawIssue> curRawIssues, Map<String, Issue> oldIssuesMap) {
        ...
        List<RawIssue> preRawIssueList = coreService.matchNewIssuesWithParentIssues(newCurRawIssues, curFile2PreFileMap.get(parentCommit), preCommitsForParent, repoUuid, analyzer, jGitHelper.getRepoPath());
        ...
        for (RawIssue preRawIssue : preRawIssues) {
            ...
            if (preRawIssue.isMapped()) {
                ...
                // todo find how to solved
                //SolveWayEnum solveWayEnum = coreService.checkHowToSolved(analyzer, jGitHelper, preRawIssue, diffs);
