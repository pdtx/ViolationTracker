package cn.edu.fudan.violation.core.solved;
...
import cn.edu.fudan.violation.mapper.RawIssueMatchInfoMapper;
import cn.edu.fudan.violation.util.AstParserUtil;
import cn.edu.fudan.violation.util.DiffFile;
import cn.edu.fudan.violation.util.JGitHelper;
import cn.edu.fudan.violation.util.pojo.TwoValue;
...
public class IssueSolved {
  ...
  private static RestInterfaceManager rest;
...
//           String repoPath = "E:\\repository\\t-repo\\cim";
  ...
  public Map<String, SolveWayEnum> checkHowToSolved(
    ...
    // todo  确认通过 AstParserUtil  解析到的方法明与 解析 location 中 anchorNode 的唯一标识是一致的
    // todo  类名没有加入进来 可能会导致部分关闭的分析不准确
    List<String> anchorNodeIdentifier = AstParserUtil.getAllMethodsInFile(curAbsoluteFilePath);
    anchorNodeIdentifier.addAll(AstParserUtil.getAllFieldsInFile(curAbsoluteFilePath));
    anchorNodeIdentifier.addAll(AstParserUtil.getAllClassNamesInFile(curAbsoluteFilePath));
