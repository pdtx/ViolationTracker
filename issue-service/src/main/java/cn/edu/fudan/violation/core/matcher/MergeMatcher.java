 ...
 */
@Slf4j
public class MergeMatcher extends BaseMatcher implements Matcher {
    ...
    private void handleMatchInfoAddAndDefault(RawIssue currentRawIssue) {
        ...
        for (RawIssueMatchInfo matchInfo : matchInfos) {
            if (matchInfo.getStatus().equals(RawIssueStatus.ADD.getType())) {
                ...
                if(issueUuid != null && coreService.checkParentCommitHasIssue(jGitHelper, addParentCommit ,issueUuid, repoUuid)){
                    //场景4
    ...
    private void handleMatchInfoDoubleAdd(List<RawIssue> currentRawIssues) {
        ...
        for (String parentCommit : parentCommits) {
            ...
            List<RawIssue> preRawIssueList = coreService.matchNewIssuesWithParentIssues(currentRawIssues.stream().
    ...
    private void handleDifferentSituationSolvedMatchInfo() {
        ...
        Map<String, List<DiffEntry>> map = new HashMap<>();
        for (String parentCommit : parentRawIssuesMap.keySet()) {
            List<DiffEntry> diffs = jGitHelper.getDiffEntry(jGitHelper.getRevCommit(parentCommit), jGitHelper.getRevCommit(currentCommit), 60);
            map.put(parentCommit, diffs);
        }
        ...
        for (Map.Entry<String, Map<String, RawIssue>> entry : issueUuid2PreRawIssues.entrySet()) {
            ...
            if (rawIssues.size() == 1) {
                // 场景 2 6 处理方法一致
                resetMatchInfoStatus(rawIssues, RawIssueStatus.MERGE_SOLVE.getType(), map);
            } else {
                // 场景 1
                resetMatchInfoStatus(rawIssues, RawIssueStatus.SOLVED.getType(), map);
            }
        }
    }

    private void resetMatchInfoStatus(Map<String, RawIssue> rawIssues, String type, Map<String, List<DiffEntry>> map) {
...
//                SolveWayEnum solveWayEnum = coreService.checkHowToSolved(analyzer, jGitHelper, rawIssueEntry.getValue(), map.get(matchInfo.getPreCommitId()));
//                matchInfo.setSolveWay(solveWayEnum.toString().toLowerCase());
            }
        }
    }
    ...
    private void updateIssueStatus() {
        ...
        for (Issue issue : issueUuid2MappedIssue.values()) {
            ...
            if (issue.getStatus().equals("Merged")){
