...
bac0d0b5-92dd-11e7-a6dd-74867af28ae8:1-60009475';
...
(
    ...
    `bug_lines`     varchar(64)  DEFAULT NULL COMMENT '表示这个bug在文件中具体体现在哪些行',
    ...
    `method_name`   varchar(256) DEFAULT NULL COMMENT 'bug所在方法名',
    `rawIssue_uuid` varchar(36)  NOT NULL COMMENT 'bug所属rawissueID',
    `code`          mediumtext COMMENT 'bug源代码',
    `offset`        int(11) NOT NULL,
    PRIMARY KEY (`uuid`) USING BTREE,
    UNIQUE KEY `uuid` (`uuid`) USING BTREE,
    KEY             `raw_issue_id_index` (`rawIssue_uuid`) USING BTREE,
    KEY             `location_method_name_index` (`method_name`)
) ENGINE = InnoDB
